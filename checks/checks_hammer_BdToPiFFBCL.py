...
"""
from slophep.Predictions.FormFactorsBToP import BdToPiFF
...
data = np.loadtxt("checks/checks_hammer_BdToPiFFBCL.txt", float, skiprows=1).T
qsq = data[0]
hammerFF_spectrum = {
    "f0"     : data[1],
    "f+"     : data[2],
...
for iff, ifflabel in zip(ff, fflabel):
    savepath = f"checks/check_hammer_BdToPiFFBCL_{iff}.png"
