...
def get_spectrum_eos(ffs: list, qsq, mode):
    ff = {}
    for iff in ffs:
        eosff = slop_to_eos[iff].format(mode)
        ffeval = [eos.Observable.make(eosff, parameters, eos.Kinematics(q2=iq2), options).evaluate() for iq2 in qsq]
        ff[iff] = np.array(ffeval)
    return ff


btod_bszeos = get_spectrum_eos(["f+", "f0", "fT"], btod_qsq, "B->D")
btodst_bszeos = get_spectrum_eos(["A0", "A1", "A12", "V", "T1", "T2", "T23"], btodst_qsq, "B->D^*")
