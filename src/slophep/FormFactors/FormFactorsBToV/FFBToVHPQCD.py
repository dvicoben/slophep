...
class FFBToV_HPQCD(FormFactorBToV):
    ...
    def get_ff(self, q2: float) -> dict[str, float]:
        ...
        A2 = (r*hA2b + hA3b)*(1.0+r)/(2.0*np.sqrt(r))
        V = hVb*(1.0+r)/(2.0*np.sqrt(r))

        T1b = -(1/(2*np.sqrt(r)))*(  (1-r)*hT2b - (1+r)*hT1b  )
        T2b = (1/(2*np.sqrt(r)))*(  2*r*(w+1)*hT1b/(1+r) - 2*r*(w-1)*hT2b/(1-r)  )
        T3b = (1/(2*np.sqrt(r)))*(  (1-r)*hT1b - (1+r)*hT2b + (1-r**2)*hT3b  )
