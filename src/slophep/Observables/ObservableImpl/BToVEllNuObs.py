...
class BToVEllNuPrediction(Observable):
    ...
    def _dJ(self, q2: float) -> dict:
        ...
        """
        mb = running.get_mb(self.pm, self.scale)
        wc = get_wceff_fccc(self.pm.get_wc(), self.pm, self._qiqj, self.lep, self.nu, mb, self.scale, nf=5)
        ...
        mB = self.get_param('m_'+self.B)
        mV = self.get_param('m_'+self.V)
        mlight = running.get_mc(self.pm, self.scale) if self._qiqj == "bc" else 0.0 # Set mlight = m_u = 0 for up quark "bu"
...
class MixinBToVAngular:
    ...
    def J(self, q2: float) -> dict:
        """Calculate rate-normalised angular coefficients
    ...
    @fluctsettings(FluctType.DICTNUMERIC)
    def uniang_obs(self, q2: float) -> dict[str, float]:
        """Calculate uniangular observables (FL, AFB, Flt, J3, J9)"""
        j = self.J(q2)
        return mt.calc_unaing_obs(j)
    
    @fluctsettings(FluctType.DICTNUMERIC)
    def binuniang_obs(self, q2min: float, q2max: float) -> dict[str, float]:
