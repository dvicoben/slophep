...
class LbToOneHalfpEllNuPrediction(Observable):
    _name = "ObsLbToOnehalfpEllNu"
    def __init__(self,
                 B: str,
                 M: str,
                 qiqj: str,
    ...
    def _prefactor(self, q2: float) -> float:
        mB = self.get_param('m_'+self._B)
        mM = self.get_param('m_'+self._M)
        ml = self.get_param('m_'+self.lep)
    ...
    def get_angularcoeff(self, q2: float) -> dict:
        ...
        wc = get_wceff_fccc_std(self.pm.get_wc(), self.pm, self._qiqj, self.lep, self.nu, mb, self.scale, nf=5)
