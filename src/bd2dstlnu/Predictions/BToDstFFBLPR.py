...
class BLPR(FormFactor):
    ...
    def get_ff_v121(self, q2: float) -> dict:
        ...
        # Hps = 1.+ash*Cps+ec*(L2+L3*(w-1)+L5-L6*(w+1))+eb*(L1-L4)
        Hv = 1.+ash*Cv1+ec*(L2-L5)+eb*(L1-L4)
        Ha1 = 1.+ash*Ca1+ec*(L2-L5*(w-1)/(w+1))+eb*(L1-L4*(w-1)/(w+1))
        Ha2 = ash*Ca2+ec*(L3+L6)
        Ha3 = 1.+ash*(Ca1+Ca3)+ec*(L2-L3-L5+L6)+eb*(L1-L4)
        Ht1 = 1.+ash*(Ct1+0.5*(w-1)*(Ct2-Ct3))+ec*(L2)+eb*(L1)
        Ht2 = 0.5*(w+1)*ash*(Ct2+Ct3)+ec*(L5)-eb*(L4)
        Ht3 = ash*(Ct2)+ec*(L6-L3)
    ...
    def get_ff_nocorr(self, q2: float) -> dict:
        """FF in BLPR parameterisation as in https://arxiv.org/pdf/1703.05330
    ...
    def get_ff_MSbar(self, q2: float) -> dict:
        """FF in BLPR parameterisation as in https://arxiv.org/pdf/1703.05330, 
        using flavio's parameters - in particular for bquark mass: mb = mb1S * (1 + 2 * alphas**2 / 9)
        with mb1S = 4.71
        ...
        w = max((mB**2 + mV**2 - q2) / (2 * mB * mV), 1)
        #paper uses 1S but this has p['mb'] = p['mb1S'] * (1 + 2 * alphas**2 / 9)
        phqet = hqet.get_hqet_parameters(self.par) 
        ash = phqet['ash']
        ec = phqet['epsc']
        eb = phqet['epsb']
        zBC = phqet['zc']
        ...
        L3 = 4.0*(chi21 + (w-1.0)*chi2p)
        L4 = 2.*(eta1 + etap*(w-1.))-1.
        # L4b = (2.0*(eta1 + etap*(w-1.0))-1.0*ebReb)
        L5 = -1.0
        # L5c = -ecRec
        L6 = -2.*(1+(eta1 + etap*(w-1.)))/(w+1.)
        # L6c = -2.0*(ecRec + (eta1 + etap*(w-1.0)))/(w+1.0)
